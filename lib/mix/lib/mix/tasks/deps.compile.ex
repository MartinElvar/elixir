defmodule Mix.Tasks.Deps.Compile do
  ...
  defp handle_rebar_not_found(%Mix.Dep{app: app, manager: manager}) do
    shell = Mix.shell
    shell.info "Could not find \"#{manager}\", which is needed to build dependency #{inspect String.to_atom(app)}"
    ...
    unless shell.yes?("Shall I install #{manager}?") do
      Mix.raise "Could not find \"#{manager}\" to compile " <>
        "dependency #{inspect String.to_atom(app)}, please ensure \"#{manager}\" is available"
    end
